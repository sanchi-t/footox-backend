...
exports.updateOrder = async (req, res) => {
  ...
  exports.getOrderUser = async (req, res) => {
    ...
    try{
      User.findOne({ _id: ObjectId(req.query._id)}).then((data) => {
        // console.log(data);
      res.status(200).json(data);
  ...
  exports.postOrderReturn = async (req, res) => {
    // console.log('hiii');
    console.log(req.body);
    const order_return = await Return.create({order_details:req.body.order,returnReason:req.body.selectedOption,comment:req.body.comment});
    ...
    try{
      User.updateOne(req.body.order,updateDoc,{upsert:true}).then((data) => {
        console.log(data);
      // res.status(200).json(data);
    }).catch((err)=>{console.log(err)});
    }
  ...
  exports.postOrderExchange = async (req, res) => {
    ...
    try{
      await Exchange.create({order_details:req.body.order,
        status: 'Exchange Initialized',
        exchange:true,
        exchange_details:exchange_details,
      ...
      await User.updateOne(req.body.order,updateDoc,{upsert:true}).then((data) => {
        // console.log(data);
    }).catch((err)=>{console.log(err)});
    }
    catch(e){
